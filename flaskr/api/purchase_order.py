  ...
  if (content_type == 'application/json'):
    ...
    for item in item_list:
      ...
      if (rows[0]['result'] != 1):
        ...
        cursor = db.execute("SELECT rowid, stock FROM ITEM WHERE name = ? AND type = ?", [item_name, item_type])
        rows = cursor.fetchall()
        initial_item_stock = rows[0]['stock']
        item_id = rows[0]['rowid']
        ...
        cursor = db.execute("SELECT stock FROM ITEM WHERE item_id = ?", [item_id])
        rows = cursor.fetchall()
